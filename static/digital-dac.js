(function(){
  ...
  function rowHtml(r,total=false){let cls=total?'dac-total':(r.level==='IDO'?'dac-ido ':'dac-lsa ')+perfClass(r);return'<tr class="'+cls+'"><td>'+esc(total?'UPSO II Total':r.label)+'</td><td>'+fmt(r.orders)+'</td><td>'+fmt(r.digital)+'</td><td>'+pct(r.digitalPct)+'</td><td>'+fmt(r.dac)+'</td><td>'+fmt(r.manual)+'</td><td>'+pct(r.dacPct)+'</td></tr>'}
  function dailyTrendHtml(selected){
    let idos=['Agra IDO','Bareilly IDO','Dehradun IDO','Noida IDO'];
    let rows=selected.map(([d,rpt])=>{let day=aggregateRows((rpt.rows||[]).filter(r=>r.level==='IDO')),map={};day.forEach(r=>map[r.label]=r);return'<tr><td class="dac-date">'+dateLabel(d)+'</td>'+idos.map(ido=>{let r=map[ido]||blankAgg(ido,'IDO',ido,d);r.digitalPct=r.orders?r.digital/r.orders:0;r.dacPct=r.orders?r.dac/r.orders:0;return'<td class="'+perfClass(r)+'"><div class="dac-mini"><b>D '+pct(r.digitalPct)+'</b><b>DAC '+pct(r.dacPct)+'</b><span>'+fmt(r.orders)+' orders</span></div></td>'}).join('')+'</tr>'}).join('');
  ...
  function barPanel(cum){
    ...
    let bars='<div class="dac-bars">'+top.map(r=>'<div class="'+perfClass(r)+'"><span>'+esc(r.label)+'</span><i><b style="width:'+Math.max(3,Math.max(r.digitalPct,r.dacPct)/max*100)+'%"></b></i><strong>D '+pct(r.digitalPct)+'</strong><strong>DAC '+pct(r.dacPct)+'</strong></div>').join('')+'</div>';
