(function(){
  ...
  window.aadhaarDashRange=JSON.parse(localStorage.indaneAadhaarDashRange||'{}');
  ...
  function normalizeAadhaarReports(){
    let fixed={};
    Object.entries(aadhaarReports||{}).forEach(([k,v])=>{
      let d=isoDate(v?.date)||isoDate(k);
      if(!d)return;
      fixed[d]={...(v||{}),date:d};
    });
    aadhaarReports=fixed;
    window.aadhaarReports=aadhaarReports;
    try{localStorage.indaneAadhaarReports=JSON.stringify(aadhaarReports)}catch(_){}
    return aadhaarReports;
  }
  ...
  function reportsInRange(from,to){
    normalizeAadhaarReports();
    from=isoDate(from);to=isoDate(to);
    return Object.entries(aadhaarReports||{}).filter(([d])=>(!from||d>=from)&&(!to||d<=to)).sort(([a],[b])=>a.localeCompare(b));
  }
  ...
  async function uploadAadhaarReport(){
    ...
    let selected=e('aadhaarDate')?.value||reportDateFromFile(file),date=isoDate(selected)||localToday();
    try{
      ...
      aadhaarDashRange={from:date,to:date};
      window.aadhaarDashRange=aadhaarDashRange;
  ...
  function setAadhaarDashRange(which,value){
    ...
    if(aadhaarDashRange.from&&aadhaarDashRange.to&&aadhaarDashRange.from>aadhaarDashRange.to){
      if(which==='from')aadhaarDashRange.to=aadhaarDashRange.from;else aadhaarDashRange.from=aadhaarDashRange.to;
    }
    window.aadhaarDashRange=aadhaarDashRange;
    try{localStorage.indaneAadhaarDashRange=JSON.stringify(aadhaarDashRange)}catch(_){}
    renderDashboard();
  ...
  function aadhaarDashboardHtml(){
    ...
    let from=isoDate(aadhaarDashRange?.from)||latest,to=isoDate(aadhaarDashRange?.to)||latest;if(from>to){let t=from;from=to;to=t}
    let selectedReports=reportsInRange(from,to),rows=aggregateReports(from,to),total=rows.filter(r=>r.level==='IDO').reduce((a,r)=>{['A','B','D','E'].forEach(c=>a[c]+=+r[c]||0);return a},blankAgg('UPSO II Total','TOTAL',''));
  ...
  function installAadhaarOverrides(){
    const oldPersist=window.persistLocalState;window.persistLocalState=function(){oldPersist&&oldPersist();try{localStorage.indaneAadhaarReports=JSON.stringify(aadhaarReports);localStorage.indaneAadhaarDashRange=JSON.stringify(aadhaarDashRange||{})}catch(_){}};
    ...
    const oldApply=window.applyPortalState;window.applyPortalState=function(d){oldApply&&oldApply(d);aadhaarReports=d?.aadhaarReports||aadhaarReports;window.aadhaarReports=aadhaarReports;normalizeAadhaarReports();persistLocalState();renderAadhaarUploadStatus()};
  ...
  function initAadhaar(){normalizeAadhaarReports();Object.assign(window,{uploadAadhaarReport,renderAadhaarUploadStatus,setAadhaarDashRange});installAadhaarOverrides();installChatHelp();ensureAadhaarUploadUi()}
  initAadhaar();
})();
